			...
			function (licenses)
				if type ~= nil then
					for i=1, #licenses, 1 do
						if type == 'driver' then
							if licenses[i].type == 'dmv' or licenses[i].type == 'drive_bike' or licenses[i].type == 'drive_truck' then
								show = true
							end
				...
				if show then
					...
					TriggerClientEvent('esx:showNotification', _source, "Вие нямата тъкъв издаден лиценз..")
				end
			end)
		end
	end)
