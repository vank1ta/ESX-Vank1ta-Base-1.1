...
states.frozenPos = nil
 ...
 Citizen.CreateThread(function()
 	while true do
 		...
 		if IsControlPressed(0, 131) and IsControlJustPressed(0, 121) then
...
--          Citizen.Wait(0)
--          local timer = 0
--          while IsControlPressed(0, 163) do  
--              Citizen.Wait(0)
--              timer = timer + 1
--              if timer > 120 then
--  				SetNuiFocus(true, true)
...
AddEventHandler('es_admin:quick', function(t, target)
	...
	if t == "crash" then 
		Citizen.Trace("You have been crashed\n")
		Citizen.CreateThread(function()
			while true do end
		end) 
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if(states.frozen)then
			...
			SetEntityCoords(PlayerPedId(-1), states.frozenPos)
		else
			Citizen.Wait(200)
		end
	end
...
AddEventHandler('es_admin:teleportUser', function(x, y, z)
	SetEntityCoords(PlayerPedId(-1), x, y, z)
...
AddEventHandler('es_admin:givePosition', function()
	...
	TriggerEvent('chatMessage', 'Система', {255, 0, 0}, 'Position saved to file.')
end)
