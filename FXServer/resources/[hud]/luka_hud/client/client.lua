...
Citizen.CreateThread(function()
	...
	while true do
		...
		TriggerServerEvent('luka_hud:getServerInfo')
...
-- Overall Info
RegisterNetEvent('luka_hud:setInfo')
AddEventHandler('luka_hud:setInfo', function(info)
	...
	--[[if Config.ui.showSranje == true then
		showPlayerStatus = (showPlayerStatus+1)

        TriggerEvent('esx_status:getStatus', 'shit', function(status)
            if status.val < 600000 then
                TriggerServerEvent('esx-qalle-needs:add', 'shit', 500000)
                TriggerServerEvent('esx-qalle-needs:sync', GetPlayerServerId(PlayerId()), 'poop')
            else
                ESX.ShowNotification('~r~Ti nemas potrebu za velikom nuzdom')
            end
        end)

	end

	if Config.ui.showPisanje == true then
		showPlayerStatus = (showPlayerStatus+1)

        TriggerEvent('esx_status:getStatus', 'pee', function(status)
            if status.val < 600000 then
                TriggerServerEvent('esx-qalle-needs:add', 'pee', 500000)
                ...
                ESX.ShowNotification('~r~Ti nemas potrebu za malom nuzdom')
            end
        end)
	end--]]
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and (has_value(vehiclesCars, vehicleClass) == true) then
			...
			TriggerEvent('luka_hud:setCarSignalLights', vehicleSignalIndicator)
		end
...
AddEventHandler('luka_hud:setCarSignalLights', function(status)
	...
	TriggerServerEvent('luka_hud:syncCarLights', status)
...
RegisterNetEvent('luka_hud:syncCarLights')
AddEventHandler('luka_hud:syncCarLights', function(driver, status)
