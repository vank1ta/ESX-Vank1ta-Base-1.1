...
Citizen.CreateThread(function()
    while true do
        ...
        if IsInRegularShopZone(coords) or IsInRobsLiquorZone(coords) or IsInYouToolZone(coords) or IsInPrisonShopZone(coords) or IsInTechShopZone(coords) then
            ...
            if IsInTechShopZone(coords) then
                if IsControlJustReleased(0, Keys["E"]) then
                    OpenShopInv("tech")
                    Citizen.Wait(2000)
                end
...
function IsInTechShopZone(coords)
    TechShop = Config.Shops.TechShop.Locations
    for i = 1, #TechShop, 1 do
        if GetDistanceBetweenCoords(coords, TechShop[i].x, TechShop[i].y, TechShop[i].z, true) < 1.5 then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
    while true do
        ...
        if GetDistanceBetweenCoords(coords, Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z, true) < 3.0 then
            ESX.Game.Utils.DrawText3D(vector3(Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z), "Натиснете ~r~[E]~s~ за да отворите магазина", 0.6)

            if IsControlJustReleased(0, Keys["E"]) then
                if Licenses['weapon'] == nil then
                    OpenBuyLicenseMenu()
                else
                    exports['mythic_notify']:SendAlert('error', 'Вече имате лиценз за оръжия!')
                end
                Citizen.Wait(2000)
            end
        end
...
function OpenBuyLicenseMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license',{
        ...
        elements = {
          { label = 'yes' ..' (' .. Config.LicensePrice ..' лв.)', value = 'yes' },
          { label = 'no', value = 'no' },
        }
      },
      function (data, menu)
...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.Shops.RegularShop.Locations) do
        CreateBlip(vector3(Config.Shops.RegularShop.Locations[k].x, Config.Shops.RegularShop.Locations[k].y, Config.Shops.RegularShop.Locations[k].z ), "Магазин - Бензиностанция", 3.0, Config.Color, Config.ShopBlipID)
    end

    for k, v in pairs(Config.Shops.RobsLiquor.Locations) do
        CreateBlip(vector3(Config.Shops.RobsLiquor.Locations[k].x, Config.Shops.RobsLiquor.Locations[k].y, Config.Shops.RobsLiquor.Locations[k].z ), "Хранителен Магазин", 3.0, Config.Color, Config.LiquorBlipID)
    end

    for k, v in pairs(Config.Shops.YouTool.Locations) do
        CreateBlip(vector3(Config.Shops.YouTool.Locations[k].x, Config.Shops.YouTool.Locations[k].y, Config.Shops.YouTool.Locations[k].z ), "Железария", 3.0, 69, Config.YouToolBlipID)
    end

    for k, v in pairs(Config.Shops.YouTool.Locations) do
        CreateBlip(vector3(Config.Shops.PrisonShop.Locations[k].x, Config.Shops.PrisonShop.Locations[k].y, Config.Shops.PrisonShop.Locations[k].z), "Затворски магазин", 3.0, Config.Color, Config.PrisonShopBlipID)
    end

    for k, v in pairs(Config.Shops.TechShop.Locations) do
        CreateBlip(vector3(Config.Shops.TechShop.Locations[k].x, Config.Shops.TechShop.Locations[k].y, Config.Shops.TechShop.Locations[k].z), "eMAG", 3.0, 1, Config.TechShopBlipID)
    end
    ...
    while true do
        ...
        for k, v in pairs(Config.Shops.RegularShop.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.RegularShop.Locations[k].x, Config.Shops.RegularShop.Locations[k].y, Config.Shops.RegularShop.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.RegularShop.Locations[k].x, Config.Shops.RegularShop.Locations[k].y, Config.Shops.RegularShop.Locations[k].z + 1.0), "Натисни ~r~[E]~s~ за да отвориш магазина", 0.6)
            end
        end
        ...
        for k, v in pairs(Config.Shops.RobsLiquor.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.RobsLiquor.Locations[k].x, Config.Shops.RobsLiquor.Locations[k].y, Config.Shops.RobsLiquor.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.RobsLiquor.Locations[k].x, Config.Shops.RobsLiquor.Locations[k].y, Config.Shops.RobsLiquor.Locations[k].z + 1.0), "Натисни ~r~[E]~s~ за да отвориш магазина", 0.6)
            end
        end
        ...
        for k, v in pairs(Config.Shops.YouTool.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.YouTool.Locations[k].x, Config.Shops.YouTool.Locations[k].y, Config.Shops.YouTool.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.YouTool.Locations[k].x, Config.Shops.YouTool.Locations[k].y, Config.Shops.YouTool.Locations[k].z + 1.0), "Натисни ~r~[E]~s~ за да отвориш магазина", 0.6)
            end
        end
        ...
        for k, v in pairs(Config.Shops.PrisonShop.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.PrisonShop.Locations[k].x, Config.Shops.PrisonShop.Locations[k].y, Config.Shops.PrisonShop.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.PrisonShop.Locations[k].x, Config.Shops.PrisonShop.Locations[k].y, Config.Shops.PrisonShop.Locations[k].z), "Натисни ~r~[E]~s~ за да отвориш магазина", 0.6)
            end
        end

        for k, v in pairs(Config.Shops.TechShop.Locations) do
            if GetDistanceBetweenCoords(coords, Config.Shops.TechShop.Locations[k].x, Config.Shops.TechShop.Locations[k].y, Config.Shops.TechShop.Locations[k].z, true) < 3.0 then
                ESX.Game.Utils.DrawText3D(vector3(Config.Shops.TechShop.Locations[k].x, Config.Shops.TechShop.Locations[k].y, Config.Shops.TechShop.Locations[k].z + 1.0), "Натисни ~r~[E]~s~ за да отвориш магазина", 0.6)
            end
        end
    end
end)
